...
const Navbar = ({ setShowCommandPalette }) => {
    ...
    const NavLink = ({ to, label }) => (
        <Link
            to={to}
            className={`relative text-sm font-medium transition-colors ${location.pathname === to ? 'text-white' : 'text-slate-400 hover:text-white'}`}
        >
            {label}
            ...
            >
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    ...
                    <Link to="/" className="hover:opacity-80 transition-opacity group relative">
                        <Logo size="md" />
                        {location.pathname === '/' && (
                            <motion.div layoutId="logo-underline" className="absolute -bottom-5 left-0 right-0 h-[2px] bg-gradient-to-r from-cyan-400 to-indigo-400" />
                        )}
                    </Link>
                            ...
                            <div className="pt-2">
                                {isConnected ? (
                                    <div className="flex items-center justify-between">
                                        ...
                                        <button onClick={handleDisconnect} className="flex items-center gap-2 text-red-400 text-sm hover:text-red-300">
                                            <LogOut size={16} />
                                            Disconnect
                                        </button>
                                    </div>
                                    ...
                                    <button
                                        onClick={handleConnect}
                                        className="w-full py-3 bg-gradient-to-r from-cyan-600 to-indigo-600 text-white font-bold rounded-xl"
                                    >
